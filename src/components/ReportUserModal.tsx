...
export function ReportUserModal({
  ...
  const handleSubmit = async () => {
    if (!selectedCategory) {
      showToast.error('Atenção', 'Selecione uma categoria');
      return;
    }

    if (!description.trim()) {
      showToast.error('Atenção', 'Descreva o problema');
      return;
    }

    try {
      setSubmitting(true);
      const response = await userApi.reportUser(targetUsername, {
      ...
      console.error('[ReportUserModal] Erro ao denunciar:', error);
      ...
      <View style={[styles.overlay, { paddingTop: insets.top }]}>
        <View style={styles.container}>
          ...
          <View style={styles.header}>
            <Text style={styles.title}>Denunciar Perfil</Text>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            <Text style={styles.subtitle}>
              Por que você está denunciando o perfil de @{targetUsername}?
            </Text>
